...
const Users = () => {
    ...
    useEffect(() => {
        const fetchUsers = async () => {
            try {
                const response = await fetch('https://webdev-dramaku-production-72b3.up.railway.app/api/users'); // Fetch only non-deleted users
                if (!response.ok) {
    ...
    const handleChangeRoleToAdmin = async (username) => {
        ...
        if (window.confirm(`Are you sure you want to change the role of ${username} to Admin?`)) {
            try {
              const response = await fetch(`https://webdev-dramaku-production-72b3.up.railway.app/api/users/${username}/role`, {
                    method: 'PUT',
                    headers: {
    ...
    const handleBanned = async (username) => {
      ...
      if (window.confirm(`Are you sure you want to ban ${username}?`)) {
          ...
          try {
              const response = await fetch(`https://webdev-dramaku-production-72b3.up.railway.app/api/users/${username}/ban`, {
                  method: 'PUT',
                  headers: {
                      'Content-Type': 'application/json',
                  },
