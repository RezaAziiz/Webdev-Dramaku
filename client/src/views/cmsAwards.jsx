...
const CmsAwards = () => {
  ...
  const handleSaveClick = async () => {
    try {
      await fetch(`https://webdev-dramaku-production-72b3.up.railway.app/awards/${editId}`, {
        method: 'PUT',
  ...
  console.log("Submitting award:", JSON.stringify(formattedData));
    ...
    try {
      ...
      const response = await fetch("https://webdev-dramaku-production-72b3.up.railway.app/awards", {
        method: "POST",
        headers: {
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmDelete) {
      try {
        const response = await fetch(`https://webdev-dramaku-production-72b3.up.railway.app/awards/${id}`, {
          method: 'DELETE',
        });
